	...
	public Login login(String username, String password, String role) {
		...
		try {
				...
				if (role.compareTo("customer") == 0) {
					result = stmt.executeQuery("select Email,PassWord from Customer");
					while (result.next()) {
						if (result.getString("Email").compareTo(username) == 0
								&& result.getString("PassWord").compareTo(password) == 0) {
					...
					result= stmt.executeQuery("select Email,PassWord,EmployeeType from Employee");
					while (result.next()) {
						if (result.getString("Email").compareTo(username) == 0
								&& result.getString("PassWord").compareTo(password) == 0&&result.getString("EmployeeType").compareTo(role)==0) {
							System.out.println("Passowrd is correct!");
							login.setRole(role);
							login.setUsername(username);
							login.setPassword(password);
